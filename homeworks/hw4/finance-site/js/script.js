...
// --- ЛОГИКА ТЕМ И SVG-СПРАЙТА (ЛАБА 3 И 4) ---
function initTheme() {
...
function applyTheme(theme) {
    ...
    const toggleBtn = document.getElementById('theme-toggle');
    if (toggleBtn) {
        // Подставляем иконку из SVG-спрайта вместо текстового эмодзи
        const iconName = theme === 'dark' ? 'icon-sun' : 'icon-moon';
        toggleBtn.innerHTML = `<svg class="icon"><use href="img/sprite.svg#${iconName}"></use></svg>`;
        ...
        if (theme === 'dark') {
            ...
            // Если мы на странице логина/регистрации, делаем кнопку тёмной
